module FX.Spec.AspNetCore.Tests.Program

open System
open FX.Spec.Core
open FX.Spec.AspNetCore.Tests

/// Execute a single TestNode and capture its result metadata.
let rec private executeNode (node: TestNode) : TestResultNode =
    ...
    | AfterEachHook _
    | AfterAllHook _ ->
        // Hooks are exercised by FX.Spec.Runner; this lightweight harness simply records them.
        GroupResult("hook", [], TestMetadata.empty)

/// Render a TestResultNode tree to the console using simple ANSI colouring.
...
/// Run and report on a named spec suite comprised of one or more TestNode roots.
...
let main _ =
    let banner lines =
        printfn "╔═══════════════════════════════════════════════════════════╗"
        lines |> List.iter (fun line -> printfn "║ %-55s ║" line)
        printfn "╚═══════════════════════════════════════════════════════════╝"

    banner
        [ "FX.Spec.AspNetCore Test Suite"
          "BDD helpers for ASP.NET Core + FX router testing"
          "Powered by FX.Spec 🧪" ]

    try
        let suites =
            [ "FxContextHelpers", FxContextHelpersSpecs.specs
              "FxRouterHelpers", FxRouterHelpersSpecs.specs ]
        ...
        if allPassed then
            banner [ "All FX.Spec.AspNetCore specs passing! 🎉" ]
            0
        else
            banner [ "Some FX.Spec.AspNetCore specs failed. ❌" ]
            1
    with ex ->
        printfn ""
        banner [ "Spec execution failed with an unhandled exception. ❌" ]
