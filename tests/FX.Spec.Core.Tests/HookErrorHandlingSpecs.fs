    ...
    describe "Hook Error Handling" [
        ...
        context "beforeAll failures" [
            ...
            it "continues running other test groups after beforeAll failure" (fun () ->
                ...
                let testNodes = [
                    describe "group 1 with failing beforeAll" [
                        beforeAll (fun () -> failwith "beforeAll failed!")
                        it "test 1" (fun () -> ())
                    ]
                    describe "group 2 should still run" [
                        it "test 2" (fun () -> group2Ran <- true)
                    ]
                ]

                let _ = Executor.executeTests testNodes
            ...
            it "provides clear error message for beforeAll failures" (fun () ->
                ...
                | TestResult.Fail (Some ex) :: _ ->
                    ...
                    expectBool(ex.Message.Contains("beforeAll") || ex.Message.Contains("Database connection failed")).toBeTrue()
                | TestResult.Fail None :: _ ->
                    failwith "Expected failure with exception details"
                | _ -> failwith "Expected at least one failure"
            )
        ]

        context "afterAll failures" [
            ...
            it "still runs tests even if afterAll will fail" (fun () ->
                ...
                let _ = Executor.executeTests [testNode]
        ...
        context "error message quality" [
            it "includes hook type in error messages" (fun () ->
                let testNode = describe "suite" [
                    beforeAll (fun () -> failwith "Setup error")
                    it "test" (fun () -> ())
                ]
