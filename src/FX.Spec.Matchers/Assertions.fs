...
type Expectation<'a when 'a: equality>(actual: 'a) =
    ...
    member _.toBeNull() =
        ...
        | null -> ()
        | _ -> raise (AssertionException("Expected null, but found non-null value", Some null, Some(box actual)))

    /// Asserts that the actual value is not null.
    /// Usage: expect(actual).notToBeNull()
    member _.notToBeNull() =
        match box actual with
        | null -> raise (AssertionException("Expected non-null value, but found null", None, Some null))
        | _ -> ()
    ...
    member _.toBeOfType<'T>() =
        ...
        | null ->
            ...
            let msg = sprintf "Expected type %s, but found null" expectedType.Name
            raise (AssertionException(msg, Some(box expectedType.Name), Some null))
        | _ ->
            let actualType = actual.GetType()
            let expectedType = typeof<'T>

            let msg =
                sprintf "Expected type %s, but found type %s" expectedType.Name actualType.Name
...
type OptionExpectation<'a when 'a: equality>(actual: 'a option) =
    ...
    /// Asserts that the Option is Some (with any value).
    /// Usage: expectOption(actual).toBeSome()
    member _.toBeSome() =
        match actual with
        | Some _ -> ()
        | None ->
            let msg = "Expected Some, but found None"
            raise (AssertionException(msg, Some(box "Some"), Some(box None)))

    /// Asserts that the Option is Some with the expected value.
    /// Usage: expectOption(actual).toBeSome(42)
    ...
    /// Usage: expectOption(actual).toBeNone()
    member _.toBeNone() =
        match actual with
        | None -> ()
        | Some value ->
            raise (
                AssertionException(sprintf "Expected None, but found Some %A" value, Some(box None), Some(box value))
            )
...
type ResultExpectation<'a, 'b when 'a: equality and 'b: equality>(actual: Result<'a, 'b>) =
    ...
    /// Usage: expectResult(result).toBeOk()
    member _.toBeOk() =
        match actual with
        | Ok _ -> ()
        | Error err ->
            let msg = sprintf "Expected Ok, but found Error %A" err
            raise (AssertionException(msg, Some(box "Ok"), Some(box err)))

    /// Asserts that the Result is Error (with any value).
    /// Usage: expectResult(result).toBeError()
    member _.toBeError() =
        match actual with
        | Error _ -> ()
        | Ok value ->
            let msg = sprintf "Expected Error, but found Ok %A" value
            raise (AssertionException(msg, Some(box "Error"), Some(box value)))
...
type BoolExpectation(actual: bool) =
    ...
    member _.toBeTrue() =
        if actual then
            ...
            raise (AssertionException("Expected true, but found false", Some(box true), Some(box false)))
    ...
    member _.toBeFalse() =
        if not actual then
            ...
            raise (AssertionException("Expected false, but found true", Some(box false), Some(box true)))
...
type FloatExpectation(actual: float) =
    ...
    member _.toBeCloseTo(expected: float, tolerance: float) =
        ...
        if diff <= tolerance then
            ...
            let msg =
                sprintf
                    "Expected %f to be close to %f (within %f), but difference was %f"
                    actual
                    expected
                    tolerance
                    diff
...
type CollectionExpectation<'a when 'a: equality>(actual: 'a seq) =
    ...
    member _.toContain(expected: 'a) =
        if Seq.contains expected actual then
            ...
            let msg =
                sprintf "Expected collection to contain %A, but it did not. Collection: %A" expected items
    ...
    member _.toHaveLength(expected: int) =
        ...
        if actualLength = expected then
            ...
            let msg =
                sprintf "Expected collection to have length %d, but found length %d" expected actualLength
    ...
    member _.toHaveCountAtLeast(expected: int) =
        ...
        if actualLength >= expected then
            ...
            let msg =
                sprintf "Expected collection to have at least %d items, but found %d" expected actualLength
    ...
    member _.toHaveCountAtMost(expected: int) =
        ...
        if actualLength <= expected then
            ...
            let msg =
                sprintf "Expected collection to have at most %d items, but found %d" expected actualLength
    ...
    member _.toContainAll(expected: 'a seq) =
        ...
        if Seq.isEmpty missing then
            ...
            let msg =
                sprintf "Expected collection to contain all items, but missing: %A" missingList
    ...
    member _.toStartWithSeq(expected: 'a seq) =
        ...
        if actualStart = expectedList then
            ...
            let msg =
                sprintf "Expected sequence to start with %A, but got %A" expectedList actualStart
    ...
    member _.toEndWithSeq(expected: 'a seq) =
        ...
        if expectedList.Length > actualList.Length then
            let msg =
                sprintf "Expected sequence to end with %A, but the sequence was shorter: %A" expectedList actualList

            raise (AssertionException(msg, Some(box expectedList), Some(box actualList)))

        let actualEnd =
            actualList |> List.skip (List.length actualList - List.length expectedList)

        if actualEnd = expectedList then
            ()
        else
            let msg =
                sprintf "Expected sequence to end with %A, but got %A" expectedList actualEnd
...
type StringExpectation(actual: string) =
    ...
    member _.toHaveLength(expected: int) =
        ...
        if actualLength = expected then
            ...
            let msg =
                sprintf "Expected string to have length %d, but found length %d" expected actualLength
    ...
    member _.toEqualIgnoreCase(expected: string) =
        if System.String.Equals(actual, expected, System.StringComparison.OrdinalIgnoreCase) then
            ...
            let msg =
                sprintf "Expected string to equal %A (ignoring case), but found %A" expected actual
...
module Assertions =
    ...
    let expectResult (actual: Result<'a, 'b> when 'a: equality and 'b: equality) : ResultExpectation<'a, 'b> =
        ResultExpectation(actual)
    ...
    let expectThrows<'TException when 'TException :> exn> (f: unit -> unit) : unit =
        try
            ...
            raise (
                AssertionException(
                    sprintf "Expected %s to be thrown, but no exception was thrown" expectedType.Name,
                    Some(box expectedType.Name),
                    None
                )
            )
        with
        ...
        | ex ->
            ...
            if actualType = expectedType then
                ...
                raise (
                    AssertionException(
                        sprintf "Expected %s, but %s was thrown" expectedType.Name actualType.Name,
                        Some(box expectedType.Name),
                        Some(box actualType.Name)
                    )
                )
    ...
    let expectThrowsWithMessage (expectedMessage: string) (f: unit -> unit) : unit =
        try
            ...
            raise (
                AssertionException(
                    sprintf "Expected exception with message %A, but no exception was thrown" expectedMessage,
                    Some(box expectedMessage),
                    None
                )
            )
        with
        ...
        | ex ->
            raise (
                AssertionException(
                    sprintf "Expected exception with message %A, but got message %A" expectedMessage ex.Message,
                    Some(box expectedMessage),
                    Some(box ex.Message)
                )
            )
    ...
    let expectThrowsContaining (substring: string) (f: unit -> unit) : unit =
        try
            ...
            raise (
                AssertionException(
                    sprintf "Expected exception containing %A, but no exception was thrown" substring,
                    Some(box substring),
                    None
                )
            )
        ...
        | ex ->
            raise (
                AssertionException(
                    sprintf "Expected exception containing %A, but got message %A" substring ex.Message,
                    Some(box substring),
                    Some(box ex.Message)
                )
            )
    ...
    let expectNotToThrow (f: unit -> unit) : unit =
        ...
        with ex ->
            raise (
                AssertionException(
                    sprintf "Expected no exception, but %s was thrown: %s" (ex.GetType().Name) ex.Message,
                    None,
                    Some(box ex)
                )
            )
