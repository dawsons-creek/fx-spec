...
module Program =
    ...
    let rec parseArgs (args: string list) (options: Options) : Options =
        match args with
        | [] -> options
        | "--filter" :: filter :: rest -> parseArgs rest { options with Filter = Some filter }
        | "-f" :: filter :: rest -> parseArgs rest { options with Filter = Some filter }
        | "--format" :: "simple" :: rest -> parseArgs rest { options with Format = Simple }
        | "--format" :: "documentation" :: rest -> parseArgs rest { options with Format = Documentation }
        | "--format" :: "doc" :: rest -> parseArgs rest { options with Format = Documentation }
        | "--verbose" :: rest -> parseArgs rest { options with Verbose = true }
        | "-v" :: rest -> parseArgs rest { options with Verbose = true }
        | "--seed" :: seedStr :: rest ->
                        ...
                        if List.isEmpty tests then
                            ...
                            if List.isEmpty filteredTests then
                                ...
                                let exitCode =
                                    ...
                                    | Documentation -> DocumentationFormatter.render results
                                    | Simple ->
