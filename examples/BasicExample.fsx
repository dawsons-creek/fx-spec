...
let multipleDescribeSpec =
    spec {
        yield describe "Stack" [
            it "starts empty" (fun () ->
...
simpleSpec |> List.iter (fun node ->
    ...
    | Group (desc, children) ->
        ...
        children |> List.iter (fun child ->
            ...
            | Group (desc, _) -> printfn "    Group: %s" desc
        )
    | Example (desc, _) ->
...
nestedSpec |> List.iter (fun node ->
    match node with
    | Group (desc, children) ->
        printfn "  Group: %s" desc
        children |> List.iter (fun child ->
            match child with
            | Group (desc2, children2) ->
...
printfn "Simple spec: %d examples, %d groups" 
    (simpleSpec |> List.sumBy TestNode.countExamples)
    (simpleSpec |> List.sumBy TestNode.countGroups)

printfn "Nested spec: %d examples, %d groups"
    (nestedSpec |> List.sumBy TestNode.countExamples)
    (nestedSpec |> List.sumBy TestNode.countGroups)
